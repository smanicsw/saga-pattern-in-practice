...
describe("GET /api/v1/payments/by-order/:orderId", () => {
  ...
  describe("Success", () => {
    it("should successfully find a payment by order id", async () => {
      ...
      const findOneResponse = await api.payments.findOneByOrderId({
      ...
      if (!findOneResponse.body.success) {
        throw new Error("Expected payment lookup by order id to succeed.");
      }
