...
export async function confirmReservation({
  ...
  const reservation = await inventoryAdapter
    .path(
      `${INVENTORY_SERVICE_API_PREFIX}/reservations/{reservationId}/confirm`,
    )
    .post<Reservation>({
...
export async function releaseReservation({
  ...
  const reservation = await inventoryAdapter
    .path(
      `${INVENTORY_SERVICE_API_PREFIX}/reservations/{reservationId}/release`,
    )
    .post<Reservation>({
      params: { reservationId },
      headers: buildOutboxEventHeaders({ outboxEventMetadata }),
...
function transformProduct({ product }: { product: Product }): Product {
  return {
    id: product.id,
    sku: product.sku,
    name: product.name,
