...
import toStringValue from "./to-string-value.js";
...
export type OutboxQueryBuilder = Knex | Knex.Transaction;

export type OutboxEventRepository<TService extends string = string> =
  ReturnType<typeof createOutboxEventRepository<TService>>;

export function createOutboxEventRepository<TService extends string>({
  getQueryBuilder,
  tableName = "outbox_events",
}: {
  getQueryBuilder: () => OutboxQueryBuilder;
  tableName?: string;
}) {
  async function createOne<TPayload>({
    outboxEvent,
  }: {
    outboxEvent: NewOutboxEvent<TPayload, TService>;
  }): Promise<OutboxEvent<TPayload, TService>> {
    const db = getQueryBuilder();

    const outboxEventRowToCreate = transformToRow({ outboxEvent });

    const [createdOutboxEventRow] = (await db<OutboxEventRow<TService>>(
      tableName,
    )
      .insert(outboxEventRowToCreate)
      .returning(OUTBOX_EVENT_RETURNING_COLUMNS)) as OutboxEventRow<TService>[];

    return transformFromRow<TPayload, TService>({
      outboxEventRow: createdOutboxEventRow,
    });
  }

  async function findManyPendingForPublishing({
    limit,
    now,
  }: {
    limit: number;
    now: string;
  }): Promise<OutboxEvent<unknown, TService>[]> {
    const db = getQueryBuilder();

    const outboxEventRows = (await db<OutboxEventRow<TService>>(tableName)
      ...
      .forUpdate()
      .skipLocked()
      .limit(limit)) as OutboxEventRow<TService>[];
  ...
  async function markOneAsPublished({
    outboxEventId,
    markOutboxEventAsPublished,
  }: {
    outboxEventId: OutboxEventId;
    markOutboxEventAsPublished: MarkOutboxEventAsPublished;
  }): Promise<OutboxEvent<unknown, TService> | null> {
    const db = getQueryBuilder();
    ...
    const [updatedOutboxEventRow] = (await db<OutboxEventRow<TService>>(
      tableName,
    )
  ...
  async function markOneAsFailed({
    outboxEventId,
    markOutboxEventAsFailed,
  }: {
    outboxEventId: OutboxEventId;
  ...
  return {
    createOne,
    findManyPendingForPublishing,
    markOneAsPublished,
    markOneAsFailed,
  };
}

function transformToRow<TPayload, TService extends string>({
  outboxEvent,
}: {
  outboxEvent: NewOutboxEvent<TPayload, TService>;
}): NewOutboxEventRow<TService> {
  return {
...
>({
  ...
  outboxEventRow: OutboxEventRow<TService>;
